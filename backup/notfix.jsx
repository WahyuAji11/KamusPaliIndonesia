...
import dictionary2 from './assets/full_dictionary' // Fixed typo in variable name
  ...
  const loadDarkMode = async () => {
   ...
   const handleSearch = useCallback((text) => {
    ...
    if (trimmedText === "") {
      ...
      // Filter results from the first dictionary
      const filteredResults1 = dictionary.filter(
        (entry) =>
          entry.pali.toLowerCase().includes(trimmedText) || entry.indonesia.toLowerCase().includes(trimmedText),
      )

      // Filter results from the second dictionary
      const filteredResults2 = dictionary2.filter(
        (entry) =>
          entry.pali.toLowerCase().includes(trimmedText) || entry.indonesia.toLowerCase().includes(trimmedText),
      )

      // Combine results from both dictionaries
      const combinedResults = [...filteredResults1, ...filteredResults2]
